...
chunk_dist = 8
nether_dist = MAX_DIST_FROM_ORIGIN // 8

# print(nether_dist)

for x in range(0, nether_dist):
    # for y in range(0, nether_dist * 2):
        # if x % 64 == 0 and y % 64 == 0:
        # if x == 216 and y == 0:
    throws.append((x * 8, 0, 0))
        # df['x_{}_y_{}'] = 
...
for throw in throws:
    ...
    # df['dist_closest'] = np.minimum(df['dist_0'], df['dist_1'], df['dist_2'])

    # conditions = [
    #     df['dist_0'] < df['dist_1'] and df['dist_0'] < df['dist_2'],
    #     df['dist_1'] < df['dist_0'] and df['dist_1'] < df['dist_2'],
    #     df['dist_2'] < df['dist_0'] and df['dist_2'] < df['dist_1']
    # ]
    ...
    # df.loc[df['dist_0'] < df['dist_1']]['dist_closest'] = df['dist_0']

    # df['dist_closest'] = np.select(conditions, outputs)

    df['dist_closest'] = df[['dist_0','dist_1','dist_2']].min(axis=1)

    median_dist = df['dist_closest'].median()

    df['dist_minus_median'] = df['dist_closest']-median_dist

    df['is_top50'] = df['dist_minus_median'] < 0

    df['dist_top50'] = df['is_top50'] * df['dist_closest']

    df.replace(to_replace={"dist_top50":0}, value=np.nan, inplace=True)

    median_top50_dist = df['dist_top50'].median(skipna=True)

    df['dist_minus_median_top50'] = df['dist_closest']-median_top50_dist

    df['is_top25'] = df['dist_minus_median_top50'] < 0

    df['dist_top25'] = df['is_top25'] * df['dist_closest']

    df.replace(to_replace={"dist_top25":0},value=np.nan, inplace=True)

    avg_top25_dist = df['dist_top25'].mean(skipna=True)

    avg_top50_dist = df['dist_top50'].mean(skipna=True)

    avg_dist = df['dist_closest'].mean()
    ...
    dist_nether = NETHER_RATIO * np.sqrt(nether_x**2 + nether_z**2)

    weighted_dist = avg_dist + dist_nether
    weighted_dist_top50 = avg_top50_dist + dist_nether
    weighted_dist_top25 = avg_top25_dist + dist_nether

    throw_dists[ind] = [nether_x, nether_z, weighted_dist_top25, weighted_dist_top50, weighted_dist, avg_top25_dist, avg_top50_dist, avg_dist]
...
throw_dist_df = pd.DataFrame.from_dict(throw_dists, orient='index', columns=['x','z','wgt_dist_25','wgt_dist_50','wgt_dist_100','avg_dist_25', 'avg_dist_50', 'avg_dist'])

print(throw_dist_df.index)

print(throw_dist_df.columns)

throw_dist_df.sort_values(by='wgt_dist_100',ascending=True,inplace=True)

print(throw_dist_df.head(20))

throw_dist_df.to_json(Path.cwd() / 'points.json')
