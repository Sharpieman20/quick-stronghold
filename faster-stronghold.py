...
for j in range(0, 3):
    ...
    df['x_{}'.format(j)] = np.cos(df['angle_{}'.format(j)])

    df['x_{}'.format(j)] *= df['strength_{}'.format(j)]

    df['y_{}'.format(j)] = np.sin(df['angle_{}'.format(j)]) 

    df['y_{}'.format(j)] *= df['strength_{}'.format(j)]
...
for throw in throws:
    ...
    for x_sub in range(-15000, 15000):
        x = x_sub/10
        y = (a*x + c) / (-b)
